...
- [ ] **Ancora da fare**: aggiungere su quella pagina Secrets sia `RESEND_API_KEY` che `SEND_EMAIL_HOOK_SECRET` (due righe separate), e `RESEND_API_KEY` anche su Vercel env vars + redeploy. Vedi valori su ECCOLO.
- [ ] **Test hook rimandato**: nessuna funzione "password dimenticata" esiste nel codice (verificato, nessun match `resetPasswordForEmail`/"forgot" in `apps/web/src`) — quindi il test va fatto con una nuova registrazione reale (es. `agospe+test1@gmail.com`) invece che via reset password. In attesa che l'utente la faccia per poter controllare i log auth/edge-function.
