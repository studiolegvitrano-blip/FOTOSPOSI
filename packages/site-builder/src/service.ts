import { createClient, generateText } from '@fotosposi/core';
...
export async function generateSiteText(eventId: string, prompt: string, section: string = 'home'): Promise<{ text?: AiGeneratedText; error?: string }> {
  const supabase = createClient();
  const result = await generateText(
    `Genera testo per sezione "${section}" di un sito di matrimonio. Prompt: ${prompt}. Tono professionale ed elegante.`,
    undefined, 300,
  );
  const generated = result.content || `[${section.toUpperCase()}] ${prompt}`;
