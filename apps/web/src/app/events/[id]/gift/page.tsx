...
export default function GiftRegistryPage() {
  ...
  return (
    <main style={{ maxWidth: 700, margin: '2rem auto', padding: '0 1rem' }}>
      <h1 style={{ marginBottom: '0.5rem' }}>{t('gift_registry')}</h1>
      <p style={{ color: '#666', marginBottom: '2rem' }}>
        {t('total_raised')} {(total / 100).toFixed(2)} EUR
      </p>

      <div style={{ marginBottom: '2rem', padding: '1.5rem', border: '1px solid #ddd', borderRadius: 8 }}>
        <h2 style={{ marginBottom: '1rem' }}>{t('gift_contribute')}</h2>
        <form onSubmit={handleContribute}>
          <div style={{ marginBottom: '1rem' }}>
            <label style={{ display: 'block', marginBottom: '0.25rem' }}>{t('your_name')}</label>
            <input
              type="text"
              value={fromName}
          ...
          <div style={{ marginBottom: '1rem' }}>
            <label style={{ display: 'block', marginBottom: '0.5rem' }}>{t('gift_amount')}</label>
          ...
          <div style={{ marginBottom: '1rem' }}>
            <label style={{ display: 'block', marginBottom: '0.25rem' }}>{t('gift_message')}</label>
            <textarea
              value={message}
          ...
          >
            {loading ? t('redirecting') : t('gift_send')}
          </button>
        </form>
      </div>
      ...
      {transactions.length > 0 && (
        <div>
          <h2 style={{ marginBottom: '1rem' }}>{t('contributions_received')}</h2>
          {transactions.map((tx) => (
            <div key={tx.id} style={{ padding: '0.75rem', border: '1px solid #eee', borderRadius: 6, marginBottom: '0.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <p style={{ fontWeight: 500 }}>{(tx.amount / 100).toFixed(2)} EUR</p>
                <small style={{ color: '#999' }}>{new Date(tx.created_at).toLocaleDateString('it-IT')}</small>
              </div>
              {tx.message && <p style={{ color: '#666', fontSize: '0.9rem', marginTop: '0.25rem' }}>"{tx.message}"</p>}
            </div>
          ))}
        </div>
      )}
