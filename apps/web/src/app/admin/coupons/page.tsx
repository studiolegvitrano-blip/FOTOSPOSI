...
import { listCoupons, createCoupon, validateCoupon, type Coupon } from '@fotosposi/commerce';
...
export default function AdminCouponsPage() {
  ...
  const loadCoupons = async () => {
    const { coupons: c } = await listCoupons();
    if (c) setCoupons(c);
  };
      ...
      {showForm && (
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input value={code} onChange={e => setCode(e.target.value.toUpperCase())}
                  className="w-full border rounded p-2 text-sm" placeholder="ES: WED2026" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" value={discountValue} onChange={e => setDiscountValue(e.target.value)}
                  className="w-full border rounded p-2 text-sm" placeholder="25" />
              </div>
              <div>
                <label className="text-sm font-medium">Max utilizzi (opz)</label>
                <input type="number" value={maxUses} onChange={e => setMaxUses(e.target.value)}
                  className="w-full border rounded p-2 text-sm" placeholder="Illimitato" />
              </div>
              <div>
            ...
            <Button onClick={handleCreate}>Crea Coupon</Button>
          </CardContent>
        </Card>
      )}
